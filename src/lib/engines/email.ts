...
/**
 * Deterministic email classification with a confidence score.
 * Used as the offline fallback; AI classification (when enabled) is stored
 * alongside and user corrections always win (userCorrected flag).
 */
...
/** Learn from corrections: returns keyword hints extracted from corrected examples. */
export async function correctionHints(userId: string): Promise<Map<EmailCategory, string[]>> {
  const { prisma } = await import("@/lib/db");
  const corrected = await prisma.emailMessage.findMany({
    where: { userId, userCorrected: true },
    select: { subject: true, category: true },
    take: 200,
  });
  const map = new Map<EmailCategory, string[]>();
  for (const c of corrected) {
    const words = c.subject.toLowerCase().split(/\W+/).filter((w) => w.length > 4);
